package com.my.baselib.lib.utils.image;
...
import static com.my.baselib.lib.utils.image.Preconditions.checkNotNull;

/**
 * create by xd at 2017/3/20
 * description:
 */
public class ImageManager {

    private static final int FIRST_GEAR = 1;
    private static final int SECOND_GEAR = 2;
    ...
    private File mFile;
    private int gear = SECOND_GEAR;
    private String filename;

    private long mSize = 100;//期望大小，默认为100
    ...
    private static synchronized File getPhotoCacheDir(Context context) {
        return getPhotoCacheDir(context, ImageManager.DEFAULT_DISK_CACHE_DIR);
    }
    ...
    * */
    ...
    public static ImageManager get(Context context) {
        if (INSTANCE == null) INSTANCE = new ImageManager(ImageManager.getPhotoCacheDir(context));
        return INSTANCE;
    }

    public ImageManager load(File file) {
        mFile = file;
        return this;
    }

    public ImageManager putSize(int size) {
        this.mSize = size;
        return this;
    }

    public ImageManager setCompressListener(OnCompressListener listener) {
        ...
        Observable.just(mFile)
                .map(new Func1<File, File>() {
                    ...
                    public File call(File file) {
                        return sizeCompress(file,mSize);
                    }
                })
    ...
    * */
    ...
    /***********************************************************************************************/
    public ImageManager putGear(int gear) {
        this.gear = gear;
        return this;
    }

    public ImageManager launch() {
        checkNotNull(mFile, "the image file cannot be null, please call .load() before this method!");
        if (compressListener != null) compressListener.onStart();
        if (gear == ImageManager.FIRST_GEAR)
            Observable.just(mFile)
                    .map(new Func1<File, File>() {
                        @Override
                        public File call(File file) {
        ...
        else if (gear == ImageManager.SECOND_GEAR)
            Observable.just(mFile)
                    .map(new Func1<File, File>() {
                        @Override
                        public File call(File file) {
    ...
    private File sizeCompress(@NonNull File file, long targetSize) {
        ...
        if (scale <= 1 && scale > 0.5625) {
            if (height < 1664) {
                ...
                size = size < targetSize ? size : targetSize;
    ...
    private File secondCompress(@NonNull File file) {
        String thumb = mCacheDir.getAbsolutePath() + File.separator +
                (TextUtils.isEmpty(filename) ? System.currentTimeMillis() : filename) + ".jpg";
